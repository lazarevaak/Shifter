    ...
    func createSet(request: CreateSet.Request) {
        guard let appDelegate = UIApplication.shared.delegate as? AppDelegate else {
            DispatchQueue.main.async {
                let response = CreateSet.Response(success: false, errorMessage: "Ошибка приложения")
        ...
        newSet.owner = currentUser
        
        do {
            try context.save()
        } catch {
            DispatchQueue.main.async {
                let response = CreateSet.Response(success: false, errorMessage: "Не удалось сохранить набор: \(error.localizedDescription)")
                self.presenter?.presentCreateSet(response: response)
            }
            return
        }
        
        // Генерируем карточки на основе введенного текста
        generateFlashcards(from: request.text) { flashcards in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    let response = CreateSet.Response(success: false, errorMessage: "Ошибка сохранения карточек: \(error.localizedDescription)")
                    self.presenter?.presentCreateSet(response: response)
                }
            }
        }
